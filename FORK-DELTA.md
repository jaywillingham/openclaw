...
| File                             | Patch                                                                                                                                                                   | Date deployed | Why                                                                                                                                                                                     |
| -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `src/agents/skills/workspace.ts` | Prompt internment via SHA-1 content-hash 16-entry LRU cache. Wraps `prompt: prompt` → `prompt: internPrompt(prompt)` in `buildWorkspaceSkillSnapshot`. ~30 lines added. | 2026-05-02    | Krang OOM root cause: identical skillsSnapshot prompts duplicated 257× per session, retaining ~1.7 GiB. Internment dedupes string identity → 60% RSS reduction confirmed via heap diff. |

**Action item:** commit the prompt-internment patch onto a `krang/prompt-internment-skills-snapshot-2026-05-02` branch before next rebase. (Tracked separately in `projects/launch-readiness/FOLLOWUP-TICKETS.md` if needed.)
...
for b in krang/codex-oauth-jwt-skip-refresh-20260428 \
         ...
         krang/origin-scoped-message-guard-20260425; do
  git checkout "$b"
